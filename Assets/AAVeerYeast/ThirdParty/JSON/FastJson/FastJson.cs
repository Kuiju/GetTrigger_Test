...
public static partial class FastJson
{
    ...
    static FastJson()
    {
        ...
        // 20171213 songlingyi
        _Register(typeof(JSONObject), _SerializeJsonObject, _DeserializeJsonObject);
    ...
    // 20171213 songlingyi
    static void _SerializeJsonObject(StringBuilder sb, object json)
    {
        ...
        jsonText = JsonUtils.JsonStringUnescape(jsonText);

        // songlingyi 去除特殊操作
        if (jsonText != null)
        {
            // songlingyi 此处添加特殊逻辑
            // 如果 jsonObj 本身是 string 则理解为是 jsonString 并 trim 掉前后的 "
            // 目前 下载功能中 由于 VideoInfo -> JsonObject 实现的不完善
            // 有直接将 jsonString 植入 JsonObject 的操作
            if (jsonObj.IsString)
    ...
    public static T Deserialize<T>(JSONObject json)
    {
        string jsonText = json.ToString();
        return Deserialize<T>(jsonText);
    }

    public static object Deserialize(JSONObject json, System.Type type)
    {
        string jsonText = json.ToString();
        return Deserialize(jsonText, type);
    }
    ...
    public class JsonParser
    {
        ...
        public string ReadJsonObject()
        {
            int startPos = count;
            SkipValue();
            return jsonText.Substring(startPos, count - startPos);
        }
        ...
        public string ReadString()
        {
            ...
            // songlingyi 临时解决 Unicode to 汉字 的问题

            //return subStringBuilder.ToString();
            return Regex.Unescape(subStringBuilder.ToString());
...
#pragma warning restore 219
    ...
    static object _DeserializeFloat(System.Type type, JsonParser parser)
    {
        ...
        System.Single.TryParse(parser.ReadToken(), out f);
        return f;
    }
    ...
    static object _DeserializeDouble(System.Type type, JsonParser parser)
    {
        ...
        System.Double.TryParse(parser.ReadToken(), out d);
        return d;
    }
    ...
    // 20171213 songlingyi
    static object _DeserializeJsonObject(System.Type type, JsonParser parser)
    {
        if (parser.ReadIfNull())
        {
            return null;
        }
        return JSONObject.Create(parser.ReadJsonObject());
    }

    // 20171211 songlingyi
    // 由 JsonTargetAttribute 标签控制的 部分序列化/反序列化 功能
    static public bool IsSimpleJsonType(System.Type inType)
    {
        return inType.IsDefined(typeof(JsonTargetAttribute), true);
    }

    static public void SimpleSerialize(StringBuilder sb, object obj)
    {
        if (_SerializeIfNull(sb, obj))
        {
            return;
        ...
        if (!fieldSerializerCache.TryGetValue(type, out fieldSerializerList))
        {
            ...
            for (int fieldIndex = 0; fieldIndex < fields.Length; ++fieldIndex)
            {
                ...
                if (!info.IsDefined(typeof(JsonTargetAttribute), true))
                {
                    continue;
                }
                ...
                string name = JsonTargetAttribute.GetJsonName(info);
    ...
    static public object SimpleDeserialize(System.Type type, JsonParser parser)
    {
        if (parser.ReadIfNull())
        {
            return null;
        ...
        for (int i = 0; i < fieldCount; i++)
        {
            ...
            if (!fieldInfo.IsDefined(typeof(JsonTargetAttribute), true))
            {
                continue;
            }

            string name = JsonTargetAttribute.GetJsonName(fieldInfo);
            if (string.IsNullOrEmpty(name))
            {
                name = fieldInfo.Name;
            }
    ...
    // 20171212 songlingyi
    // 由 JSONObject 反序列化 临时方案
    // TODO: 去掉中间的 JSONObject.ToString() 中转步骤，从方法实现层面整合 JSONObject 和 FastJson；
    public static T DeserializeJson<T>(JSONObject json)
    {
        if (json.IsNull)
        {
            return default(T);
