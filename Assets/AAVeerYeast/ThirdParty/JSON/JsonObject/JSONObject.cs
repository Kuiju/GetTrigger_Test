...
public partial class JSONObject
{
    ...
    private static JSONObject CreateNull()
    {
#if POOLING
    ...
    {
        JSONObject bakedObject = CreateNull();
    ...
    void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
    {
        if (!string.IsNullOrEmpty(str))
        {
            ...
            if (str.Length > 0)
            {
                ...
                {
                    type = Type.STRING;
                    this.str = JsonUtils.JsonStringUnescape(str.Substring(1, str.Length - 2));
                }
                else
                {
                    int tokenTmp = 1;
    ...
    #region songlingyi 补充添加类型 for Add

    public void Add(System.Int64 val)
    {
        Add(Create(val));
    }
    ...
    public void Add(System.UInt16 val)
    {
        Add(Create(val));
    }

    public void Add(System.UInt32 val)
    {
        Add(Create(val));
    }
    ...
    public void Add(System.Byte val)
    {
        Add(Create(val));
    }
    ...
    #region songlingyi 补充添加类型 for AddField
    ...
    public JSONObject AddField(string name, System.UInt16 val)
    {
        return AddField(name, Create(val));
    }

    public JSONObject AddField(string name, System.UInt32 val)
    {
        return AddField(name, Create(val));
    }
    ...
    public JSONObject AddField(string name, System.Byte val)
    {
        return AddField(name, Create(val));
    }
    ...
    {
        return AddField(name, CreateStringObject(val));
    }
    ...
    public JSONObject SetField(string name, string val) { return SetField(name, CreateStringObject(val)); }
    public JSONObject SetField(string name, bool val) { return SetField(name, Create(val)); }
    public JSONObject SetField(string name, float val) { return SetField(name, Create(val)); }
    public JSONObject SetField(string name, int val) { return SetField(name, Create(val)); }
    ...
    {
        return Create(Print(false, false));
    }
    /*
...
#pragma warning disable 219
    public string Print(bool escapeString, bool pretty)
    {
        StringBuilder builder = new StringBuilder();
        Stringify(escapeString, 0, builder, pretty);
...
#pragma warning restore 219
     ...
     */
    void Stringify(bool escapeString, int depth, StringBuilder builder, bool pretty = false)
    {   //Convert the JSONObject into a string
        //Profiler.BeginSample("JSONprint");
        if (depth++ > MAX_DEPTH)
        {
        ...
        switch (type)
        {
            ...
            case Type.STRING:
                if (escapeString)
                    builder.AppendFormat("\"{0}\"", JsonUtils.JsonStringEscape(str));
                else
                    builder.AppendFormat("\"{0}\"", str);
                break;
            case Type.NUMBER:
...
#endif
                            builder.AppendFormat("\"{0}\":", key);
                            obj.Stringify(escapeString, depth, builder, pretty);
                            builder.Append(",");
#if (PRETTY)
                            if (pretty)
                                builder.Append("\n");
                        ...
                        if (list[i] as JSONObject != null)
                        {
                            ...
                            if (pretty)
                                for (int j = 0; j < depth; j++)
                                    builder.Append("\t"); //for a bit more readability
#endif
                            list[i].Stringify(escapeString, depth, builder, pretty);
                            builder.Append(",");
#if (PRETTY)
                            if (pretty)
                                builder.Append("\n"); //for a bit more readability
    ...
    public string ToString(bool escapeString, bool pretty = false)
    {
        return Print(escapeString, pretty);
    }
    ...
    public static implicit operator JSONObject(double n)
    {
        return Create((float)n);
    }
    public static implicit operator double(JSONObject d)
    {
        return (d == null) ? 0 : d.f;
    }

    public static implicit operator JSONObject(float n)
    {
        return Create(n);
    }
    public static implicit operator float(JSONObject d)
    {
        return (d == null) ? 0 : d.f;
    }

    public static implicit operator JSONObject(int n)
    {
        return Create(n);
    }
    public static implicit operator int(JSONObject d)
    {
        return (d == null) ? 0 : (int)d.i;
    }
    ...
    public static implicit operator bool(JSONObject d)
    {
        return (d == null) ? false : d.b;
    }
}
